...
form by SZ operator.  Symm_sz name comes from the fact that the alpha-alpha
and beta-beta blocks are not using antisymmetric basis functions as is common in the
2-RDM literature.  Because antisymmetric basis functions are used we must add
antisymmetric constraints to the alpha-alpha, beta-beta blocks and also constrain
the appropriate elements of this block to be zero.
"""
import numpy as np
